...
def test_kubectl_run_oneshot_passes_command_flag_and_pod_name(tmp_path: Path) -> None:
    """`run_oneshot` must:

      * Pass the positional `NAME` (pod name) before
        any other flag — `kubectl run NAME` is required.
      * Include `--command --` between the namespace
        and the container command so `/bin/sh` (which
        fails RFC 1123) is treated as the container's
        `command:` field instead of the autogenerated
        `containers[0].name`. Without `--command` the
        first arg after `--` is auto-named
        `containers[0].name` and k8s rejects it.
      * Use a pod name that satisfies RFC 1123
        (lowercase alphanumeric + `-`).

    This is the regression guard for the smoke-probe
    bug found during the WP15 live-cluster smoke test
    on 2026-07-15.
    """
    runner, fake = _make_runner(tmp_path)
    runner.run_oneshot(
    ...
    assert "run" in args_list
    # The pod name is the positional directly after `run`.
    assert args_list[args_list.index("run") + 1] == "cicd-smoke"
    assert "--image=curlimages/curl:8.10.1" in args_list
    assert "--namespace=gitea" in args_list
    # `--command` immediately before the `--` separator.
    assert "--command" in args_list
    sep_idx = args_list.index("--")
    assert args_list[sep_idx - 1] == "--command"
    # Container command starts directly after `--`.
    assert args_list[sep_idx + 1] == "/bin/sh"
    assert args_list[sep_idx + 2] == "-c"
